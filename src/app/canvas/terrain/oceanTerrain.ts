...
export function getTerrainElevation(x: number, localY: number): number {
  const h1 = Math.sin(x * 0.035 + localY * 0.025) * 5.0;            // Main rolling ridges
  const h2 = Math.cos(x * 0.08 - localY * 0.06) * 3.5;              // Mounds and depressions
  const h3 = Math.sin(x * 0.15 + localY * 0.12) * 1.8;              // Medium sediment bumps
  const h4 = Math.sin(x * 0.018) * Math.cos(localY * 0.022) * 5.5;  // Regional highlands vs lowlands
  return h1 + h2 + h3 + h4;
}
